...
namespace Facebook
{
    ...
    {
		public virtual string GetPage(string uri)
		{
			return (string)Api(HttpMethod.Get, "", null, null, uri);
		}
        ...
        protected virtual object Api(HttpMethod httpMethod, string path, object parameters, Type resultType, string uriOverride = "")
        {
            Stream input;
            bool containsEtag;
            IList<int> batchEtags;
            var httpHelper = PrepareRequest(httpMethod, path, parameters, resultType, out input, out containsEtag, out  batchEtags, uriOverride);
